...
$sql =<<<INICIOSQL
select * from servidor left join pernoite on (pernoite.id_servidor=servidor.id_servidor) inner join os on (pernoite.id_os=os.id_os) inner join os_debito on (os_debito.id_os=os.id_os) inner join identificador on (identificador.id_identificador=os.id_identificador) inner join exercicio on (exercicio.id_exercicio=identificador.id_exercicio and exercicio.ano=$anosql) inner join posto on (posto.id_posto=servidor.id_posto) inner join especialidade on (especialidade.id_especialidade=servidor.id_especialidade) inner join fase on (os.id_fase=fase.id_fase) where fase.id_fase  not in (26,27,29,31,71)  order by os.id_os desc,os.saida_data desc, servidor.cpf asc
INICIOSQL;
...
//       $localizadores .= $dado['LOCALINICIAL']."->".$dado['LOCALIZADOR'].' ';
     ...
     if(mysql_query('update pernoite set passagem_valor='.$passagemvalor.', passagem_solicitacao_num="'.$passagemsolicitacao.'" where id_pernoite='.$dados['id_pernoite'])){
     	echo "OK\n";     	
     	echo 'update pernoite set passagem_valor='.$passagemvalor.', passagem_solicitacao_num="'.$passagemsolicitacao.'" where id_pernoite='.$dados['id_pernoite'].'<br>';
     }; 
