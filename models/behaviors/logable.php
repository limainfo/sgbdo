...
class LogableBehavior extends ModelBehavior 
{
	...
	function findLog(&$Model, $params = array()) {
		...
		if ($params[ $this->settings[$Model->alias]['userName'] ] && $this->UserModel ) {
			$options['conditions'][$this->settings[$Model->alias]['userName']] = $params[ $this->settings[$Model->alias]['userName'] ];
		}
	...
	function setUserData(&$Model, $userData = null) {
		if ($userData) {
			$this->user = $userData;
			$this->setUserIp(&$this->UserModel,$_SERVER['REMOTE_ADDR']);
		}
	}
    ...
    function _saveLog(&$Model, $logData, $title = null) {  
    	...
    	if (isset($this->Log->_schema[ $this->settings[$Model->alias]['userName'] ]) && $this->user) {
    		$logData['Log'][$this->settings[$Model->alias]['userName']] = $this->user[$this->UserModel->alias][$this->UserModel->nome];
    	}
        ...
        if (isset($this->Log->_schema['description'])) {
    		...
    		$logData['Log']['created'] = date('d-m-Y H:i:s');
