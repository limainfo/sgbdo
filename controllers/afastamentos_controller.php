...
// Seleciona escalas em que o militar já esteja cadastrado
			...
			if((preg_match($p601, $t01))||(preg_match($p701, $t01))||(preg_match($p801, $t01))||(preg_match($p901, $t01))){
				...
				if(!(empty($conflitos))){
					...
					$mensagem .= '<li>O militar já foi escalado '.count($conflitos).' vez(es). Para cadastrar o afastamento é necessário substitui o militar conforme dados abaixo :.'.$problema.'</li>';print_r($this->data);
				}
			}
...
//					$this->Session->setFlash(__('Os dados de  Afastamento foram gravados.', true));
	...
	function delete($id = null) {
		...
		//$this->layout='admin';
		if (!$id) {
			$this->Session->setFlash(__('ID inválido para Afastamento', true));
			$this->redirect(array('action'=>'index'));
		}
		//Configure::write(array('debug'=> 2));
		...
		$mudanca = '<u><b>Afastamento Excluído:</b></u>'.$rsetor;
		$monitora = 'insert into logs (title, created, model, action, usuario_nome, ip, changes) values ("Exclusão de Afastamento",now(),"AFASTAMENTO", "'.$mudanca.'","'.$usuario.'", "'.$ip.'", "'.$mudanca.'")';
									//		echo $monitora;
		//		exit();
		...
		if ($this->Afastamento->delete($id)) {
			$this->Session->setFlash(__('Afastamento excluído', true));
                        echo json_encode('Afastamento excluído');
			//$this->redirect(array('action'=>'index'));
		}
        exit();
	}
...
// Seleciona escalas em que o militar já esteja cadastrado
			...
			if($status==0){
				...
				//		$mensagem .= 'Os dados de Afastamento para a escala <b>'.$nomeEscala.'</b> não foram gravados. Por favor, tente novamente.<br>';
				//}else{
...
//		$this->set('afastamentosdodia',$afastamentospelousuario);
	...
	function externoexcluiafastamentos(){
		...
		if ($this->Afastamento->delete($id)) {
			$this->Session->setFlash(__('Afastamento excluído:'.$excluido['Afastamento']['motivo'].'->'.$excluido['Afastamento']['dt_inicio'],true));
		}		
