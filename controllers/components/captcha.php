...
//caminho de captcha.php  /www/operacional/controllers/components
$caminho = substr(__FILE__, 0, strrpos(__FILE__, '/'));
$caminho = str_replace('controllers/components','vendors',$caminho);


App::import('Vendor','Securimage' ,array('file'=>'securimage/securimage.php')); //use this with the 1.2 core
//App::import('Vendor','Securimage' ,array('file'=>$caminho.'/securimage/securimage.php')); //use this with the 1.2 core

 
//the local directory of the vendor used to retrieve files

define('CAPTCHA_VENDOR_DIR',  APP . 'vendors' . DS . 'securimage/');
//define('CAPTCHA_VENDOR_DIR',  $caminho . '/securimage/');
...
class CaptchaComponent extends Object {
    ...
    var $_image_bg_color = "#808080"; //the background color for the image
   ...
   // var $_wordlist_file = 'words/words.txt'; //the wordlist to use
	...
	//$this->_gd_font_file = CAPTCHA_VENDOR_DIR.$this->_gd_font_file;
		...
		//CaptchaComponent instance of controller is replaced by a securimage instance
         $options = array(
              'code_length' => $this->_code_length,
              'num_lines' => 3,
              'noise_level' => 6
             
          );
		$controller->captcha =& new securimage($options);
