...
if(!class_exists('FPDF'))
{
	...
	class FPDF
	{
		var $tmpFiles = array(); //Incluído para suportar canal alpha
		//Private properties
		...
		function Close()
		{
			...
			//Close document
			$this->_enddoc();
			foreach($this->tmpFiles as $tmp) @unlink($tmp); //Incluído para suportar canal alpha			
		}
		...
		function ImageJpg($file,$x,$y,$w=0,$h=0,$type='',$link='')
		{
...
//Incluído para suportar canal alpha
...
function _gamma($v){
	return pow ($v/255, 2.2) * 255;
		...
		if (isset($info["masked"])) $this->_out('/SMask '.($this->n-1).' 0 R'); //Incluído para suportar canal alpha
    ...
    return array('w'=>$a[0], 'h'=>$a[1], 'cs'=>$colspace, 'bpc'=>$bpc, 'f'=>'DCTDecode', 'data'=>$data);
		...
		function _parsepng($file)
		{
			...
			$colspace='Indexed';
				...
				fclose($f);
				return 'alpha'; //Incluídos para suportar canal alpha
			}
			if(ord(fread($f,1))!=0)
