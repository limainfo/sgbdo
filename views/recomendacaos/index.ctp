...
<div class="recomendacaos index">
<h1><?php __('Recomendações de Segurança');?>
<?php
$script = "var x=\$('find').value;if(x.blank()){\$('broffice').href='".$this->webroot."recomendacaos/indexExcel/';}else{\$('broffice').href='".$this->webroot."recomendacaos/indexExcel/'+x;}";
...
<tr>
	...
	<th><?php echo $paginator->sort('localidade');?></th>
	<th><?php echo $paginator->sort('setor');?></th>
	<th><?php echo $paginator->sort('data_recomendacao');?></th>
	<th><?php echo $paginator->sort('numero_recomendacao');?></th>
	<th><?php echo $paginator->sort('prazo_do');?></th>
	<th><?php echo $paginator->sort('providencia');?></th>
...
foreach ($recomendacaos as $recomendacao):
	...
	if($recomendacao['Recomendacao']['status']=='PENDENTE'){
		$class = ' style="background-color:#f0d0d0;"';
	}
?>
	<tr>
		<td<?php echo $class;?>>
		...
		<td class="actions" <?php echo $class;?>>
			<?php echo $this->Html->link($this->Html->image('lupa.gif', array('alt'=> __('Exibir', true), 'border'=> '0', 'title'=>'Visualizar')), array('action'=>'view', $recomendacao['Recomendacao']['id']),array('escape'=>false, 'escape'=>false), null,false); ?>
			<?php echo $this->Html->link($this->Html->image('lapis.gif', array('alt'=> __('Editar', true), 'border'=> '0', 'title'=>'Editar')), array('action'=>'edit', $recomendacao['Recomendacao']['id']),array('escape'=>false, 'escape'=>false), null,false); ?>
			<?php
			echo $this->Html->link($this->Html->image('lixo.gif', array('alt'=> __('Excluir', true), 'border'=> '0', 'title'=>'Excluir')), array(), array('onmousedown'=>"dialogo('Deseja realmente excluir o registro #".$recomendacao['Recomendacao']['documento']." ?' ,'".$this->webroot.$this->params['controller'].'/delete/'.$recomendacao['Recomendacao']['id']."');",'onclick'=>"this.href='#';return false;",'escape'=>false, 'escape'=>false), null,false); 
		?></td>
	</tr>
<?php endforeach; ?>
</table>
