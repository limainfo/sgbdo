...
<div class="eptaEptas index">
	<h1><?php __('Epta Eptas');?>	&nbsp;<?php echo $html->link($html->image('broffice.png', array('alt'=> __('BROffice', true), 'border'=> '0', 'title'=>'Dados em planilha BROffice', 'onmouseover'=>"\$('busca').action='".$this->webroot."eptaEptas/indexExcel/';", 'onclick'=>'$("busca").submit();' )), '#', array('id'=>'broffice','escape'=>false), null,false); ?>	
    &nbsp;<?php echo $html->link($html->image('novo.gif', array('alt'=> __('Cadastrar', true), 'border'=> '0', 'title'=>'Cadastrar')), array('action'=>'add', null),array('escape'=>false, 'escape'=>false), null,false); ?>	</h1>
	<?php echo $form->create('formFind', array('url' => 'index','type'=>'file','action' => 'index','controller' => 'eptaEptas','id'=>'busca','onsubmit'=>'sql();')); ?>	
	<h3>
	<?php
...
<?php
		...
		echo '</select>&nbsp;&nbsp;&nbsp;<img border="0" onclick="ShowContent(\'filtro\');"  title="Filtrar Dados" alt="Filtro" src="'.$this->webroot.'img/filtro.gif"  onmouseover="$(\'busca\').action=\''.$this->webroot.'eptaEptas/index/\';"/></div>'; ?>
...
?>
	...
	<tr>
			...
			<th><?php echo $this->Paginator->sort('base_indic_loc_id');?></th>
			...
			<th><?php echo $this->Paginator->sort('portaria');?></th>
			<th><?php echo $this->Paginator->sort('portaria_dt');?></th>
			<th><?php echo $this->Paginator->sort('bca');?></th>
			<th><?php echo $this->Paginator->sort('bca_dt');?></th>
			<th class="actions"><?php __('Ações');?></th>
	</tr>
	<?php
	$i = 0;
	foreach ($eptaEptas as $eptaEpta):
		$class = null;
		if ($i++ % 2 == 0) {
			$class = ' class="altrow"';
		}
	?>
	<tr<?php echo $class;?>>
		<td><?php echo $eptaEpta['EptaEpta']['id']; ?>&nbsp;</td>
		<td>
		...
		<td><?php echo $eptaEpta['EptaEpta']['entidade']; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['cidade']; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['local']; ?>&nbsp;</td>
		<td>
			<?php echo $this->Html->link($eptaEpta['Estado']['nome'], array('controller' => 'estados', 'action' => 'view', $eptaEpta['Estado']['id'])); ?>
		</td>
		<td><?php echo $categorias[$eptaEpta['EptaEpta']['categoria']]; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['portaria']; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['portaria_dt']; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['bca']; ?>&nbsp;</td>
		<td><?php echo $eptaEpta['EptaEpta']['bca_dt']; ?>&nbsp;</td>
		<td class="actions">

		<?php echo $html->link($html->image('lupa.gif', array('alt'=> __('Exibir', true), 'border'=> '0', 'title'=>'Visualizar')), array('action'=>'view', $eptaEpta['EptaEpta']['id']),array('escape'=>false, 'escape'=>false), null,false); ?>

		<?php echo $html->link($html->image('lapis.gif', array('alt'=> __('Editar', true), 'border'=> '0', 'title'=>'Editar')), array('action'=>'edit', $eptaEpta['EptaEpta']['id']),array('escape'=>false, 'escape'=>false), null,false); ?>
