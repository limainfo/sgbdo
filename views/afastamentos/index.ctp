...
<table>
<tr><th colspan="2">RELATÓRIOS</th></tr>
<tr><th>Afastamentos por Escala</th><td>
<?php
...
echo $form->end();
?></td></tr>
<tr><th>Afastamento de todo o efetivo por mês</th><td>
<?php
$a=date('Y');
for($i=$a-2;$i<=$a+2;$i++){
	$anos[$i]=$i;
...
echo $form->create('Afastamento', array('action'=>'externoexcel','onsubmit'=>'javascript:excel();return false;','type'=>'file'));
echo $form->select('mesprevisto', $meses ,null,array('class'=>'formulario'), false);
echo $form->select('anoprevisto', $anos ,null ,array('class'=>'formulario'), false).'<input type="submit" value="Obter broffice" class="botoes">';
echo $form->end();
?></td></tr>
<tr><th>Consulta por nome ou afastamento</th><td>
<?php
//print_r($escalas);
...
echo '<div class="input text"><input type="text" maxlength="100" size="30" class="formulario" id="find" value="'.$this->data['formFind']['find'].'" name="data[formFind][find]"/>
...
<table cellpadding="0" cellspacing="0">
	<tr>
		...
		<th><?php echo $paginator->sort('hora_extra');?></th>
...
function excluiafastamento(idafastamento) {
    	var idafastamentook = idafastamento.replace('tr','');
			    	
	//var dados = Form.serialize($('AfastamentoAddForm'));
	new Ajax.Request('<?php echo $this->webroot; ?>afastamentos/delete/'+idafastamentook, {
			method: 'get',
			onSuccess: function(transport) {
                            var resultado = transport.responseText.evalJSON(true);
                            alert(resultado);
			    $(idafastamento).remove();
			},
                        onCreate:function(request, xhr) {
                            var t=new Dialog({content:'<img alt="" width="15" height="15" src="/sgbdo/img/spinner.gif"> Aguarde ...',title:'Atualizando tabelas',close:{link:false,overlay:false,esc:false}});t.open();}
	})
	...
	//print_r($responsavel);
	?>
	<tr <?php echo $class;?> id='tr<?php echo $afastamento['Afastamento']['id'];  ?>'>
		<td onmouseover='EscondeDica();ExibeDica("<b>Cadastro:</b><?php echo $responsavel[$afastamento['Afastamento']['militar_responsavel']][0];?><br><b>Identidade:</b><?php echo $responsavel[$afastamento['Afastamento']['militar_responsavel']][1];?><br><b>Dt Registro:</b><?php echo date('d-m-Y h:i:s',strtotime($afastamento['Afastamento']['created']));?><br><br>");' >
...
<!-- <div  rel='tooltip' id='i<?php echo $afastamento['Afastamento']['id']; ?>' title='<?php echo '<p style="background-color:#000;color:#fff;"><br><b>Responsável:</b>'.$responsavel[$afastamento['Afastamento']['militar_responsavel']][0]."<br><b>Identidade:</b>".$responsavel[$afastamento['Afastamento']['militar_responsavel']][1].'<br><b>Dt Registro:</b>'.date('d-m-Y',strtotime($afastamento['Afastamento']['created'])).'<br><br></p>'; ?>'>
			<?php echo $this->Html->link($afastamento['Militar']['Posto']['sigla_posto'].' '.$afastamento['Militar']['Especialidade']['nm_especialidade'].' '.$afastamento['Militar']['nm_completo'], array('controller'=> 'militars', 'action'=>'view', $afastamento['Militar']['id']),array('rel'=>'tooltip')); ?>
			</div>
 -->		
		</td>
		...
		<td><?php echo nl2br($afastamento['Afastamento']['obs']); ?></td>
		<td><?php echo nl2br($afastamento['Afastamento']['hora_extra']); ?></td>
...
//		echo ($u[0]['Usuario']['militar_id']==$afastamento['Afastamento']['militar_responsavel'])?"=":"<>";
		...
		if($u[0]['Usuario']['militar_id']==$afastamento['Afastamento']['militar_responsavel']){
			$apaga=1;
		}
		//echo $apaga.' apagar';
...
//if(($u[0]['Usuario']['militar_id']==$afastamento['Afastamento']['militar_responsavel'])||($u[0]['Usuario']['privilegio_id']==1)||($u[0]['Usuario']['privilegio_id']==4)||($apaga)){
		if((strpos($u[0][0]['setores'],$afastamento['Afastamento']['setor_id'])!==false)||($u[0]['Usuario']['privilegio_id']==1)||($u[0]['Usuario']['privilegio_id']==4)||($apaga)){
...
//		echo $this->Html->link($this->Html->image('lixo.gif', array('alt'=> __('Excluir', true), 'border'=> '0', 'title'=>'Excluir')), array(), array('onmousedown'=>"dialogo('Deseja realmente excluir o registro #".$afastamento['Afastamento']['dt_inicio']." ?' ,'".$this->webroot.$this->params['controller'].'/delete/'.$afastamento['Afastamento']['id']."');",'onclick'=>"this.href='#';return false;",'escape'=>false, 'escape'=>false), null,false);
		echo $this->Html->link($this->Html->image('lixo.gif', array('alt'=> __('Excluir', true), 'border'=> '0', 'title'=>'Excluir')), array(), array('onmousedown'=>"dialogo('Deseja realmente excluir o registro #".$afastamento['Afastamento']['motivo'].' - '.$afastamento['Militar']['Posto']['sigla_posto'].' '.$afastamento['Militar']['Especialidade']['nm_especialidade'].' '.$afastamento['Militar']['nm_completo']." ?' ,'functionexcluiafastamento(\'tr".$afastamento['Afastamento']['id']."\');');",'onclick'=>"this.href='#';return false;",'escape'=>false, 'escape'=>false), null,false);
		} 
	//	}
		?>
		</td>
...
function excel() {
var mes = $('AfastamentoMesprevisto').value;
var ano = $('AfastamentoAnoprevisto').value;

window.open('{$this->webroot}afastamentos/externoexcel/'+mes+'/'+ano);
