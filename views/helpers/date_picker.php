...
class DatePickerHelper extends FormHelper {
    ...
    */   
    function picker($fieldName, $options = array()) {
        ...
        $options['after'] = $this->Html->link($this->Html->image('../js/jscalendar/img.gif'), '#', array('onClick'=>"return showCalendar('".$htmlAttributes['id']."', '".$this->format."'); return false;",'escape'=>false), null, false);
    ...
    function Timer ($fieldName, $options = array(),$formato) {
    	$this->setaFormato($format);
        $this->_setup();
        $this->setEntity($fieldName);
        $htmlAttributes = $this->domId($options);       
        $divOptions['class'] = 'date';
        $options['type'] = 'text';
        $options['div']['class'] = 'date';
        $options['after'] = $this->Html->link($this->Html->image('../js/jscalendar/img.gif'), '#', array('onClick'=>"return showCalendar('".$htmlAttributes['id']."', '".$formato."', '24', true); return false;",'escape'=>false), null, false);
    ...
    function setaFormato($format = null){
        if($format != null){
            $this->format = $format;
        }
