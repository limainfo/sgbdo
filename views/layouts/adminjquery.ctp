...
//		if(!empty($acesso['setoresassociados'])){echo("<a href='".$this->webroot.'setoresassociados/add'."'>Órgão e setores associados</a>");}
	...
	if((!empty($acesso['escalas']))||(!empty($acesso['escalasmonths']))||(!empty($acesso['turnos']))){
		...
		if(($compara==1)||($compara==4)||($compara==12)){
			echo("<a href='{$this->webroot}pimos/add'>Escala: Cadastrar PIMO</a> ");
			echo("<a href='{$this->webroot}escalas/externomilitarescala'>Chefe Regional: Cadastrar legendas</a> ");
			echo("<a href='{$this->webroot}usuarios/add'>Chefe Regional: Cadastrar usuários</a> ");
		}
		echo("<span class='qmdivider qmdividerx'></span></div>");
	}  
...
//			}
		...
		if(($compara==1)||($compara==4)||($compara==12)||($compara==11)){
