                ...
                new Ajax.Request(url + '/' + encodeURIComponent(motivo), {
                    ...
                    var st03 = '<br>'+resposta.mensagem;
                        ...
                        new Dialog({
                            ...
                            $('dialog-content').update(st03);
                                if(resposta.mensagem.length>2){
...
if(($selprev=='p')&&(!empty($escala['Escalasmonth'][0]['ok_chefep']))&&(empty($escala['Escalasmonth'][0]['destrava_prevista']))&&(file_exists($absoluto))){

    $caminho = $this->webroot.'webroot/pdf/'.$dta.$dtm.$auxilio.$selprev.'.pdf';

}

if(($selprev=='c')&&(!empty($escala['Escalasmonth'][0]['ok_chefec']))&&(empty($escala['Escalasmonth'][0]['destrava_cumprida']))&&(file_exists($absoluto))){
...
?>
<div
    style="z-index: 2; width: 100%; height: 100%; position: fixed; top: 0%; left: 0%; padding: 0px; float: center;"
    id="conflitos">
                ...
                <tbody>
                    <tr>
                        <th width="10%" class="td_esq"><a href="#"
                            onclick="$('conflitos').hide();"><img
                            src="<?php echo $this->webroot.'img/btsair.gif'; ?>"></a></th>
                        <th width="90%" class="td_dir"><b>Não foi assinado devido ao fato
                        de alguns militares estarem afastados,<br>
                        porém constam como escalados</b></th>
                    </tr>
                    <tr>
...
<div
    ...
    id="verso">
<div align="center"><?php  if($trava==0){echo $form->create('Escala', array('action'=>'verso','onsubmit'=>'submitForm(this); return false;','type'=>'file'));}
?>
...
<table width="80%" border="0" cellpadding="0" cellspacing="0"
    ...
    <tr>
        ...
        ?> <textarea name="data[Verso][adjunto_obs]" id="adjuntoobs" cols="80"
            rows="1" class="formulario"><?php echo $verso[0]['Versoescala']['adjunto_obs']; ?></textarea></td>
    </tr>
    <tr>
        <td class="td_esq" colspan="2" style="background-color: #ff8080;">Os
        itens de 2 até 5 serão obtidos do cadastro de Afastamentos. Basta
        visualizar o PDF.</td>
    </tr>
    ...
    <tr>
        ...
        <td width="90%" class="td_dir"><textarea
            name="data[Verso][naoconformidades]" id="naoconformidades" cols="80"
            rows="6" class="formulario"
            <?php if(!$escrita){echo "readonly=\"readonly\"";} ?>><?php echo $verso[0]['Versoescala']['naoconformidades']; ?></textarea></td>
    </tr>
    <?php
    }
    ?>
    ...
    <tr>
        <td colspan="2" class="td_esq"><textarea
            name="data[Verso][alteracoes]" id="alteracoes" cols="80" rows="4"
            class="formulario"><?php if($this->data['Escala']['prevista']=='p'){echo $verso[0]['Versoescala']['item2'];}else{echo $verso[0]['Versoescala']['item5'];} ?></textarea>
        </td>
    </tr>
    <tr>
    ...
    <tr>
        <td colspan="2" class="td_esq"><textarea name="data[Verso][obscmt]"
            id="obscmt" cols="80" rows="2" class="formulario"><?php if($this->data['Escala']['prevista']=='p'){echo $verso[0]['Versoescala']['item3'];}else{echo $verso[0]['Versoescala']['item6'];} ?></textarea>

        </td>
    </tr>
    <tr>
        <td colspan="2" height="1000"><?php
        //echo $form->hidden('Escalaid', array('value'=>$escala['Escala']['id']));
...
<div
    style="float: center; position: static; width: 100%; height: 100%;"><?php echo $form->create('Escala', array('controller'=> 'Escala', 'action'=>'zera'));
    echo $form->hidden('Escala.id', array('value'=>$escala['Escala']['id']));

    ?> <!--
<input type="hidden" id="EscalaDtm" value="{$mes}"
    name="data[Escala][dtm]" /> <?php
    //$this->webroot
    ...
    for ($mes=1;$mes<=12;$mes++){
         ...
         */
        //$valores ='<div style="float:left;margin: 0 auto;">';
        $valores ='';
...
VALOR;
        if($mes==$dtm){
            ...
            $valores .=<<<VALOR2
            <img border="0" onclick=" $('mes{$mes}').submit();" title="Consultar escala do mês {$mes}" alt="{$mes}" src="{$this->webroot}img/v{$mes}.png"/>
...
VALOR2;
            ...
            $valores .=<<<VALOR2
            <img border="0" onclick=" $('mes{$mes}').submit();" title="Consultar escala do mês {$mes}" alt="{$mes}" src="{$this->webroot}img/{$mes}.png"/>
...
VALOR2;
    ...
    --> <?php echo $form->hidden('Escala.opcao', array('value'=>'VER'));
    echo $form->hidden('Escala.anterior', array('value'=>'0','id'=>'anterior'));?>
<a href="#" onclick="$('verso').show(); "><img
    src="<?php echo $this->webroot.'img/verso.gif'; ?>" id="clickverso"></a>
<a href="#" onclick="listaMapa();$('mapas').show(); "><img
    src="<?php echo $this->webroot.'img/mapa.png'; ?>"
    id="clickafastamento"></a>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0  class="td_full">
    <tr>
        ...
        echo $this->Html->image($gif, array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Gerar PDF da escala atual', 'id'=>'pdf')); ?>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <?php
        if($assinaescalante){ ?> <?php //if($this->data['Escala']['prevista']=='p'){echo $this->Html->image('zerar.png', array('alt'=> __('ZERAR', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Gerar nova previsão', 'onClick'=>'var sn=confirm("Tem certeza que deseja gerar nova previsão da escala atual ?"); if(sn){if(confirm("Gerar baseado no mês anterior?")){$(\'EscalaOpcao\').value = \'ANTERIOR\';  $(\'EscalaZeraForm\').submit();}else{if(confirm("Gerar automaticamente?")){$(\'EscalaOpcao\').value = \'ATUAL\';  $(\'EscalaZeraForm\').submit();}}}'));} ?>
        <?php
        if($selprev=='c'){
            echo $this->Html->image('desfazer.jpg', array('alt'=> __('DESFAZER', true), 'border'=> '0','style'=>'float:left;', 'title'=>'Desfazer alterações da escala cumprida', 'id'=>'desfazer')); 

        }
        echo $this->Html->image('lapis.gif', array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Assinar Escala atual', 'id'=>'assinarescalante')); ?>

        <?php }
        if($limpaAssinaturaEscalante){
            echo $this->Html->image('limpa.gif', array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:left;', 'title'=>'Limpa assinaturas', 'id'=>'limpaassinaturaescalante'));
...
SCRIPT;
        ...
        ?> <br>
        <?php
        //if($this->data['Escala']['prevista']=='p'){ echo $this->Html->image('zerar.png', array('alt'=> __('ZERAR', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Gerar nova previsão', 'onClick'=>'var sn=confirm("Tem certeza que deseja gerar nova previsão da escala atual ?"); if(sn){$(\'EscalaOpcao\').value = \'ATUAL\';  $(\'EscalaZeraForm\').submit();}'));}
        if($oaple){
            echo '<br>'.$escalante;
            /*
...
SCRIPT;
        ...
        ?>&nbsp;&nbsp;&nbsp;<?php echo $this->Html->image($gif, array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Gerar PDF da escala atual', 'id'=>'pdfchf')); ?>
        <?php if($assinachefe){  ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $this->Html->image('lapis.gif', array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Assinar Escala atual', 'id'=>'assinarchf')); ?>
        <?php }
        if($limpaAssinaturaChefe){
            echo $this->Html->image('limpa.gif', array('alt'=> __('PDF', true), 'border'=> '0','style'=>'float:left;', 'title'=>'Limpa assinaturas', 'id'=>'limpaassinaturachefe'));
...
<!-- LIMITE DE DIAS -->     
        ...
        <td class="td_full"><b>HORA INSTRU&Ccedil;&Atilde;O:</b><br><?php echo $horainstrucao; ?>
        <!--<input type="text" id='EscalasmonthHoraInstrucao' name='data[Escalasmonth][hora_instrucao]' value="<?php echo $horainstrucao; ?>" class="formularios" size="3"></td>-->
...
SCRIPT;
    ...
    if(($u[0]['Usuario']['privilegio_id']==5)){
        if(empty($okescalantep)){
            if(($selprev=='p')&&(empty($okescalantep))){
                echo $this->Html->image('lixo__.gif', array('alt'=> __('LIMPAR', true), 'border'=> '0','style'=>'float:right;', 'title'=>'Limpar dados ', 'onClick'=>'var sn=confirm("Tem certeza que deseja limpar dados atuais ?"); if(sn){$(\'EscalaOpcao\').value = \'ZERAR\';  $(\'EscalaZeraForm\').submit();}'));
            }
        }
    }
    if($oaple){
...
<table width="100%" border="0" cellpadding="0" cellspacing="0"
    ...
    foreach ($legendas as $militar){
       if($escala['Escala']['tipo']=='RISAER'){
         $codigos[$militar['Militar']['id']] = $militar[0]['nome'];
       }else{
    ...
    <tr class="td_lados">
        ...
        for ($l=1;$l<=$turno['Turno']['qtd'];$l++){
            ...
            if(empty($milesc)){
                ...
                if($escala['Escala']['tipo']=='RISAER'){
                    $selecionado[$milesc] = $preenche[$cumprimento][0]['pessoaescalada'];
                }else{
...
if(!$trava){
    if($selprev=='p'){
        foreach($codigos as $chave=>$valor){
            if(empty($okescalantep) && empty($okchefep) && empty($okchefeorgaop)){
        ...
        foreach($codigos as $chave=>$valor){
            if(empty($okescalantec) && empty($okchefec) && empty($okchefeorgaoc)){
                $selectDinamico .= '<option value="'.$chave.'">'.$valor.'</option>';
            }
        }
...
<?php echo $form->create('Escala', array('action'=>'externoreserva','onsubmit'=>'enviaReserva(this); return false;','type'=>'file'));
?>
<textarea name="data[Reserva][nomes]" id="reserva" cols="80" rows="2" class="formulario"><?php echo $verso[0]['Versoescala']['reservas']; ?></textarea>
<div align="center">
...
<script type="text/javascript">
//<![CDATA[
function enviaReserva(form) {
/*
usa método request() da classe Form da prototype, que serializa os campos
do formulário e submete (por POST como default) para a action especificada no form
*/

new Ajax.Request('{$this->webroot}escalas/externoreserva/', {
            method: 'post',
            parameters: {escalasmonth: '{$escalasmonth}', escala: '{$escala['Escala']['id']}' , reserva: $('reserva').value},
            onSuccess: function(transport) {
