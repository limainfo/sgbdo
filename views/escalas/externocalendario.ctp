...
<?php
//gera calendario
 $raiz = $this->webroot.'escalas/externocalendario';
...
function calendario($raiz, $imagem, $dados){
    ...
    $retorno .= "<font style=\"font-family:verdana,arial,serif;font-size:16\"><b>CALENDÁRIO DO ESCALADO: ".converte_mes($mes)."/".$ano."</b></font><br>";
       ...
       While ($dia_semana < 7)
       {
          If ($cont_mes <= $t_mes)
          {
             If ($dia_semana == $cont_semana) //celulas de dias do mes
             {
                ...
                $nome_feriado=$feriados[$cont_mes."-".((int)$mes)];
                    $retorno.=  $nome_feriado.'<br>';
                    if(!empty($dados[strtotime($ano.'-'.$mes.'-'.$cont_mes)])){
                        foreach($dados[strtotime($ano.'-'.$mes.'-'.$cont_mes)] as $diasescalado){
                            $retorno .= "<a href='#' class='bd' ><p >{$diasescalado['escala']}</p>{$diasescalado['turno']}</a><br>";
             ...
             Else //celulas vazias no inicio (mes anterior)
             {
                ...
                $retorno.= "<font color=#AAAAAA face=verdana,arial,serif size=2>".$dia_semana_ant."</font>";
                    if(!empty($dados[strtotime($ano_ant.'-'.$mes_ant.'-'.$dia_semana_ant)])){
                        foreach($dados[strtotime($ano_ant.'-'.$mes_ant.'-'.$dia_semana_ant)] as $diasescalado){
                            $retorno .= "<a href='#' class='bd' ><p >{$diasescalado['escala']}</p>{$diasescalado['turno']}</a><br>";
          ...
          Else
          {
                While ($cont_semana < 7) //celulas vazias no fim (mes posterior)
                {
                    ...
                    if(!empty($dados[strtotime($ano_post.'-'.$mes_post.'-'.$dia_semana_post)])){
                        foreach($dados[strtotime($ano_post.'-'.$mes_post.'-'.$dia_semana_post)] as $diasescalado){
                            $retorno .= "<a href='#' class='bd' ><p >{$diasescalado['escala']}</p>{$diasescalado['turno']}</a><br>";
