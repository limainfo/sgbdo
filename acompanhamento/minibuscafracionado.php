...
$conexao = mysql_pconnect($hostSGBDO,$nomeSGBDO,$senhaSGBDO);
	...
	//$sqltemp = "insert ignore into paeatsindicadostemp (militar_id,paeat_id) values('$militar_id',$paeat_id) ";
	//$consulta = mysql_query($sqltemp);

	$sqltemp = "select * from paeatsindicados where militar_id='$militar_id' and paeat_id=$paeat_id ";
	$consulta = mysql_query($sqltemp);
	$ostemp= '';
	$matriculadotemp= '';
	$passagemtemp= '';
...
//	if(empty($ostemp)&&empty($matriculadotemp)&&empty($passagemtemp)){
	...
	$completa .= " and datediff('$inicio',os.saida_data)<=10 and datediff('$fim',os.regresso_data)>=-10  ";
...
//if(empty($ostemp)){
	...
	while($dados = mysql_fetch_array($consulta, MYSQL_BOTH)){
	   ...
	   sleep(2);
		   ...
		   //echo 'http://10.32.63.109/cpa/webservice/consultavalor/obtemdados.asp?os='.$os."&ano=".$ano."";
		   ...
		   if(!empty($objeto) ){
			...
			$conexao = mysql_pconnect($hostSGBDO,$nomeSGBDO,$senhaSGBDO);
		   	mysql_select_db($dbSGBDO,$conexao);
		   	$atualiza = "update paeatsindicados set  passagem='$localizadores', matriculado='$fase', os='$osbd' where paeat_id=$paeat_id and militar_id='$militar_id' ";
...
*/
	...
	if($insere){
		...
		$conexao = mysql_pconnect($hostSGBDO,$nomeSGBDO,$senhaSGBDO);
		mysql_select_db($dbSGBDO,$conexao);
		$atualiza = "update paeatsindicadostemp set os='$osbd', passagem='$localizadores' where paeat_id=$paeat_id and militar_id='$militar_id' ";
		mysql_query($atualiza);
	}
	*/
	$sqltemp = "select * from paeatsindicados where militar_id='$militar_id' and paeat_id=$paeat_id ";
	...
	header('Content-type: application/x-json');
	echo '{ "ok":"'.$ok.'","os":"'.$ostemp.'","passagem":"'.$passagemtemp.'","matriculado":"'.$faseos.'"}';
